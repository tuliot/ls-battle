...
var Vote = require('./models/voteTweet');
...
function trackVote(tweet) {
  ...
  db.getContestants(function(allContestants) {
    ...
    if (validTags.length) {
      console.log('Tweet by %s counted.', twitterUser.name);
      ...
      if (winningUser.votees.indexOf(twitterUser.id) === -1) {
        ...
        var vote = new Vote({
          ...
          imageURL: twitterUser.profile_image_url,
          tweetBody: tweet.text,
          tweetURL: '',
          tweetId: tweet.id,
          hashtag: winningHastag,
          voteFor: winningUser.id,
          createdAt: new Date()
        });
